...
export default async function SeriesBrowsePage({ searchParams }: { searchParams: any }) {
  ...
  const endpoint = sort === 'top_rated' ? '/tv/top_rated' : '/trending/tv/week'
  const data = await tmdbFetch(endpoint, { page })
  const items = (data.results || []).map((m: any) => ({ ...m, _type: 'tv' })).slice(0, perPage)
  ...
  const bUrl = `/series?sort=${sort}`

  return (
    <section className="section" style={{ paddingTop: 'calc(var(--header-h) + 24px)' }}>
      <div className="section-head">
        <div className="section-title"><i className="fas fa-tv icon" /> Web Series</div>
        <div style={{ display: 'flex', gap: 8 }}>
          <Link href="/series?sort=popular" className={`btn btn-sm ${sort === 'popular' ? 'btn-primary' : 'btn-dark'}`}>Popular</Link>
          <Link href="/series?sort=top_rated" className={`btn btn-sm ${sort === 'top_rated' ? 'btn-primary' : 'btn-dark'}`}>Top Rated</Link>
        </div>
      </div>
